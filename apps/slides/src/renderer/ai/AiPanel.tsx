...
import { IconClock, IconGear, IconNewChat, IconRefresh, IconSidebarCollapseLeft } from '../components/icons'
...
interface AiPanelProps {
  ...
  onOpenSettings?: () => void
  /** user-supplied skills injected into the agent loop */
...
export function AiPanel({
  ...
  if (!loopRef.current) {
    ...
    // Generation steps use the user's configured model directly (the former forced-Anthropic
    // override was removed with the foreign providers).
    // errKind marks failure categories that shouldn't retry (timeout/empty output/user stop)
    type LlmResult = {
      ok: boolean
      text?: string
      error?: string
    ...
    const access: DeckAccess = {
      ...
      generateStyleSkill: async (a) => {
        ...
        const r = await runLlmOnce(sys, userMsg, undefined, a.signal)
        return r.ok && r.text
    ...
    loopRef.current = new AgentLoop({
      ...
      events: {
        ...
        onError: (error) => {
          ...
          // login to gate on, so just finalize the failed turn.
